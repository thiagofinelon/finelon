...
/*
  ...
  Serial.println(Pluv_REEDCOUNT);//*0.2794);
  Pluv_REEDCOUNT = Pluv_REEDCOUNT * 0.25;
  Serial.println(Pluv_REEDCOUNT);
...
float get_SpeedWind() {
  ...
  float speedwind = (((4 * 3.1415926535897932 * 147 * Anem_RPM) / 60) / 1000) * 3.6; // Calculate wind speed on km/h
  return speedwind;
}
...
int get_DirVento() {
  int Dir_Vento_valor = analogRead(VENTDIRPIN);
  int Dir_Vento_Winddir;
  Dir_Vento_valor = map(Dir_Vento_valor, 0 , 1023 , 0 , 5);

  if (Dir_Vento_valor <= 0.27) {
   return Dir_Vento_Winddir = 315;
  }
  else if (Dir_Vento_valor <= 0.32) {
   return Dir_Vento_Winddir = 270;
  }
  else if (Dir_Vento_valor <= 0.38) {
   return Dir_Vento_Winddir = 225;
  }
  else if (Dir_Vento_valor <= 0.45) {
   return Dir_Vento_Winddir = 180;
  }
  else if (Dir_Vento_valor <= 0.57) {
   return Dir_Vento_Winddir = 135;
  }
  else if (Dir_Vento_valor <= 0.75) {
   return Dir_Vento_Winddir = 90;
  }
  else if (Dir_Vento_valor <= 1.25) {
   return Dir_Vento_Winddir = 45;
  }
  else {
   return Dir_Vento_Winddir = 000;
  }
}
...
void get_PluvCalc() {
  ...
  int Pluv_val = digitalRead(Pluv_REED);                      //Read the status of the Reed swtich
...
void send2SD(char payload[])
{
  ...
  file_test = SD.open(caminho, FILE_WRITE);
...
void send_data()
{
  ...
  root["uv"] = get_UV();
  root["ws_kmh"] = get_SpeedWind();
  root["wind_dir"] = get_DirVento();
  root["pluv"] = Pluv_REEDCOUNT;
  root["pm25"] = get_PM2;
  root["pm10"] = get_PM1;
  root["igh"] = get_piranometerRead();
